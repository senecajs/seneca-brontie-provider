...
This document discusses why `@seneca/brontie-provider` is built the way it is.
...
The payoff is uniformity. An application reading from Brontie Partner, a
payment processor and a CRM uses one access pattern for all three:

```js
await seneca.entity('provider/brontie/balance').list$()
...
`provider/brontie/` canon, registering the `role:entity` messages
...
*already* entity-shaped. The Brontie Partner SDK exposes accessors like
`client.Balance()` — carrying
`load` —
...
`voucher`, and the store surface those
...
`brontie` and sends the `apikey` as `authorization: Bearer <apikey>`
...
[@voxgig/sdkgen](https://github.com/voxgig/sdkgen) from the Brontie Partner API
...
in `sdk-pin.json`, which `make sdk-src` fetches to `.sdksrc/brontie-sdk` —
together with the sdkgen component that emits this target.
...
.use('@seneca/brontie-provider', {
  test: true,
  testopts: { entity: { balance: { 'balance0': { ... } } } },
})
```

This is better than the usual provider-testing compromise. Rather than checking
only that the plugin loads and answers
`sys:provider,provider:brontie,get:info`, the tests exercise the
entity commands themselves — load, the not-found answer —
